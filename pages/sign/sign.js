...
Page({
    data: {
      ...
      wx.setStorageSync('date', this.data.getDate);
      ...
      onhide:function(){
        wx.setStorageSync('continulity', this.data.continuity);
        wx.setStorageSync('ornot', this.data.ornot);
      ...
      this.data.lastdate=wx.getStorageSync('date');
      this.data.continuity= wx.getStorageSync('continulity');
      this.data.ornot=wx.getStorageSync('ornot');
      this.data.day=wx.getStorageSync('day');
      this.data.days=wx.getStorageSync('days');
      console.log(this.data.ornot);
      this.setNowDate();
      this.getProWeekList()
      this.dataTime();
      ...
      console.log(that.data.getDate);
      console.log(this.data.lastmonth);
      
      if(parseInt(that.data.getDate)!=parseInt(that.data.lastdate)||parseInt(that.data.month)!=parseInt(that.data.lastmonth)){
        this.setData({
            ornot:false
        });
    }
    ...
    console.log(this.data.ornot);
        ...
        //let ornot = that.data.ornot?0:1//当天是否签到
          ...
          for(let i = 0;i < num;i++){
              if(i <= (obinl + 1)){ 
                  if(ornot == 0){
                      if(0 <= (obinl - i - 1)){
                          that.data.day[obinl - i - 1].src = "/icon/qiandaox.png"
                      }
                  }else{
                      if(0 <= (obinl - i - 2)){
                          that.data.day[obinl - i - 2].src = "/icon/qiandaox.png"
                      }
                  }
              }else{
                  return false
      ...
      dakainc:function(e){
          ...
          that.setData({
              ornot:true,
              continuity:Number(Number(that.data.continuity) + 1),
              days:[],
